...
| **Longevity quadrant** (X = peak year, Y = longevity, size = volume) + linked time series | Directly classifies "consistent vs sudden"; the categories (Classic, Boomer, 70s fad, Current) are intuitive; click-detail completes the view. | Bubble overplotting; "longevity" must be defined (years above a threshold). |
...
| **Animated motion chart** | Encodes sex + volume, attractive. | Animation prevents comparing trajectories (memory load); cluttered, illegible. *Dropped.* |

**Chosen:** Longevity quadrant + linked time series (overview + detail). Diagonal heatmap as a strong alternative.
...
---
**Cross-cutting consistency:** reuse of the "diagonal heatmap" visual language across Viz 1 and 3;
consistent sex color encoding throughout; cleaned data (rare names, `dpt=XX`, `annais=XXXX` excluded).
